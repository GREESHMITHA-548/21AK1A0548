...
const TEST_SERVER_API = "http://example.com/api"; // Replace with the actual third-party API
...
app.get('/numbers/:numberId', async (req, res) => {
  ...
  // Add new numbers to the storage, maintaining uniqueness
  newNumbers.forEach(num => {
    if (!numbersStorage.includes(num)) {
      if (numbersStorage.length >= WINDOW_SIZE) {
        numbersStorage.shift(); // Remove the oldest number
      }
